      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          {steps.map((step, index) => (
            <div
              key={index}
              className="flex flex-col items-center text-center p-8 bg-white/90 rounded-2xl shadow-xl hover:shadow-2xl transition-shadow duration-300"
            >
              <span className="text-5xl mb-4 drop-shadow-lg">{step.icon}</span>
              <h3 className="text-xl font-bold text-emerald-700 mb-2">{step.title}</h3>
              <p className="text-gray-700 text-base mb-6 flex-grow">{step.description}</p>
              {step.link && (
                <Link
                  href={step.link}
