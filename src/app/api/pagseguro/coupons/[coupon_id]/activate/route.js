...
export async function PUT(request, { params }) {
  ...
  if (!PAGSEGURO_AUTH_TOKEN) {
    ...
    return NextResponse.json({ error: 'Configuração de pagamento (ativar cupom) indisponível.' }, { status: 500 });
  }
  ...
  try {
    ...
    // TODO: Verificar permissão do usuário para ativar este cupom

    const { coupon_id } = params;
    if (!coupon_id) {
      return NextResponse.json({ error: 'O ID do cupom é obrigatório para ativação.' }, { status: 400 });
    }

    const idempotencyKey = uuidv4();
    const activateCouponUrl = `${PAGSEGURO_COUPONS_API_BASE_URL}/${coupon_id}/activate`;

    console.log(`[PagBank Activate Coupon] Ativando cupom ID: ${coupon_id}. URL: ${activateCouponUrl}`);

    const pagBankResponse = await fetch(activateCouponUrl, {
      method: 'PUT',
      headers: {
    ...
    console.log(`[PagBank Activate Coupon] Resposta do PagBank:`, JSON.stringify(responseData, null, 2));

    if (!pagBankResponse.ok) {
      console.error(`Erro da API PagBank ao ativar cupom [${pagBankResponse.status}]:`, responseData);
      return NextResponse.json({
        error: `Falha ao ativar cupom ${coupon_id} no PagBank.`,
    ...
    return NextResponse.json({
      message: `Cupom ${coupon_id} ativado com sucesso!`,
      couponDetailsPagBank: responseData,
    });

  } catch (error) {
    console.error('Erro interno do servidor ao ativar cupom PagBank:', error);
    return NextResponse.json({ error: 'Erro interno no servidor ao ativar cupom.', details: error.message }, { status: 500 });
  }
}
