...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    // TODO: Adicionar verificação se o usuário é admin, se a ativação de planos for restrita
    ...
    if (!plan_id) {
      ...
      return NextResponse.json({ error: 'O ID do plano é obrigatório para ativação.' }, { status: 400 });
    }

    // 4. Gerar chave de idempotência
    const idempotencyKey = uuidv4();

    // 5. Montar a URL para ativar o plano
    const activatePlanUrl = `${PAGSEGURO_PLANS_API_BASE_URL}/${plan_id}/activate`;

    console.log(`[PagBank Activate Plan] Ativando plano ID: ${plan_id} na URL: ${activatePlanUrl}`);

    // 6. Fazer a requisição para a API de Ativação de Planos do PagBank
    // Esta requisição PUT não envia corpo (body)
    const pagBankResponse = await fetch(activatePlanUrl, {
      method: 'PUT',
      headers: {
    ...
    console.log(`[PagBank Activate Plan] Resposta do PagBank para ativação do plano ${plan_id}:`, JSON.stringify(responseData, null, 2));

    if (!pagBankResponse.ok) {
      console.error(`Erro da API de Planos PagBank ao ativar plano [${pagBankResponse.status}]:`, responseData);
      return NextResponse.json({
        error: `Falha ao ativar plano ${plan_id} no PagBank.`,
    ...
    return NextResponse.json({
      message: `Plano ${plan_id} ativado com sucesso no PagBank!`,
      planDetailsPagBank: responseData, // A resposta da API de ativação geralmente é o objeto do plano atualizado
    });

  } catch (error) {
    console.error('Erro interno do servidor ao ativar plano PagBank:', error);
    return NextResponse.json({ error: 'Erro interno no servidor ao ativar plano.', details: error.message }, { status: 500 });
  }
}
